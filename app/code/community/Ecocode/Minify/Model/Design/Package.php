...
class Ecocode_Minify_Model_Design_Package extends Mage_Core_Model_Design_Package
{
	...
	private function getSuffix($type){
		...
		if(!$this->suffix){
			Mage::getSingleton('ecocode_minify/log')->log('Generating new suffix (If you see this multiple times in a short time something is badly wrong!)');
			$this->suffix = time();
			//this flag is global
	...
	public function minifyJS($targetFilename){
		$sizeBefore = filesize($targetFilename);
